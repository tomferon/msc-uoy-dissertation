...
Later, \textcite{pablo_amster_towards_2009} attempted to generalise Dupire's Equation to multiple assets. They showed that the price of European \textit{basket} call options, options whose underlying is a basket of assets with fixed weights, also satisfy some partial differential equation, related to but with a different form than Dupire's Equation. Nonetheless, we will refer to it as the \textit{generalisation} of Dupire's Equation here for simplicity.

The goal of this dissertation is to present the necessary background for these results and to present them in details, taking particular care to make each step explicit.

We begin by establishing the prerequisites about stochastic processes and stochastic calculus in Section \ref{sec:preliminaries}. Most results in this section are stated without proofs but references are provided in each case. The \textit{multi-asset Black-Scholes model with variable coefficients} is then presented in Section \ref{sec:black-scholes}, where we also define many concepts and show important results to \textit{price} basket options. In Section \ref{sec:fokker-planck}, we define \textit{Markov processes} and derive the \textit{Fokker-Planck Equation} that they satisfy. This equation will then be used to prove both Dupire's Equation and its generalisation. Section \ref{sec:local-vol} presents a restriction of the model developed in Section \ref{sec:black-scholes}, the so-called \textit{local volatility model}. In this restriction, we then prove Dupire's Equation in Section \ref{sec:dupire}. Finally, we show its generalisation in Section \ref{sec:generalisation}, together with a numerical example that uses this generalisation to price basket options. The implementation of that method in C++ is provided in the appendix as well as in accompanying source files.
...
In \textcite{pavliotis_stochastic_2014}, the proof of the Fokker-Planck Equation is rather short (less than a page) and uses a different definition of It\^o diffusions. Here, we provide a more detailed proof (about 3.5 pages) based on the definition of It\^o diffusions as solutions to SDEs (see Definition \ref{def:ito-diffusion}).
...
Finally, we present a discretisation of the PDE from Theorem \ref{thm:generalisation} using a finite difference method. An implementation of the method in C++ is also provided. The code also implements Monte Carlo simulations to compare the results as a sanity check.
