...
\begin{definition}
  A (European) \textbf{derivative security} with expiry time $T$ is a security whose value is a stochastic process $H : [0,T] \times \Omega \to \mathbb{R}$ and such that $H(T) = H$ for some $\mathcal{F}^\mathbf{W}_T$-measurable random variable $H$ called the \textbf{payoff} of the derivative.
\end{definition}

\begin{definition}
  The \textbf{discounted process} of a process $X$ is defined as
  \begin{align*}
    \tilde{X}(t) = e^{-\int_0^t r(s) \mathrm{d}s} X(t).
  \end{align*}
\end{definition}
...
\begin{definition}
  We say that a strategy is \textbf{admissible} if
  \begin{itemize}
    \item there exists a constant $L > 0$ such that its value process satisfies
      \begin{align*}
        V(t) \ge -L
      \end{align*}
      for all $t \in [0,T]$, almost surely and
    \item its discounted price $\tilde{V}$ is a martingale under $Q$.
  \end{itemize}
\end{definition}
...
\begin{assumption}[No arbitrage principle]
  There exist no arbitrage opportunities in the extended market.
\end{assumption}

\begin{assumption}
  The \textbf{price process} of a derivative security with payoff $H$ is an It\^o process $H : [0,T] \times \Omega \to \mathbb{R}$ such that $H(T) = H$ and such that there are no arbitrage opportunities in the extended market.
\end{assumption}

\subsection{Pricing of derivatives}

\tomcomment{Everything after this point is still work in progress.}

\begin{definition}
  We say that an admissible self-financing strategy (in the basic market) \textbf{replicates} a derivative with payoff $H$ if $V(T) = H$.
\end{definition}

\begin{assumption}\label{ass:bs-repl-strat-derivative-prices}
  If a derivate with payoff $H$ is replicable by a strategy with value process $V$, then $V(t) = H(t)$ for all $t \in [0,T]$.
\end{assumption}

FIXME: Conditions for replicability?

\begin{theorem}
  If $(x_1, \ldots, x_d, y)$ is a replicating strategy with value $V$ for a derivative with payoff $H$, then
  ...
  \begin{proof}
    By assumption \ref{ass:bs-repl-strat-derivative-prices}, we have that $H(t) = V(t)$ for all $t \in [0,T]$. Multiplying both sides by the discounting factor $\exp\left(-\int_0^t r(s) \mathrm{d}s\right)$, we also have that $\tilde{H}(t) = \tilde{V}(t)$ for all $t \in [0,T]$.

    Since $(x_1, \ldots, x_d, y)$ is a replicating strategy, it is admissible by definition. This in turn implies that its discounted value $\tilde{V}$ is a Q-martingale. Therefore,
    \begin{align*}
      \tilde{H}(t) = \tilde{V}(t)
...
\begin{definition}
  European \textbf{call and put options} with expiry time $T$, strike price $K > 0$ and underlying $U$ a replicable $\mathcal{F}^\mathbf{W}_t$-adapted process, are derivative securities with payoffs
  \begin{align*}
    H_\text{call} = (U(T) - K)^+, && H_\text{put} = (K - U(T))^+.
  \end{align*}
\end{definition}

We need to make the following further assumption on the risk-free rate. Informally, we can assume that bonds with maturity $T$ are traded at all time $t$.

\begin{assumption}\label{ass:bs-bond-prices}
  The process $\exp\left(-\int_t^T r(s) \mathrm{d}s\right)$ is $\mathcal{F}^\mathbf{W}_t$-adapted.
\end{assumption}
...
\begin{theorem}[Put-call parity]
  ...
  \begin{proof}
    ...
    \end{align*}
    Here, we use assumption \ref{ass:bs-bond-prices} to ensure that $y(t)$ is $\mathcal{F}^\mathcal{W}_t$-adapted. The strategy's value at time $T$ is
    \begin{align*}
      V(T) = \sum_{i=1}^d x_i^U(t) S_i(t) + y^U(t) A(t) - \frac{K}{A(T)} A(T).
      = U(T) - K
    \end{align*}

    Hence, assumption \ref{ass:bs-repl-strat-derivative-prices} implies that equation \eqref{eq:put-call-parity} indeed holds.
  \end{proof}
\end{theorem}

FIXME: European options are replicable => C(0) = E(exp(...) H)
