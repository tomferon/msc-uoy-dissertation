...
\begin{align*}
  ...
  \end{bmatrix},
\end{align*}
for some parameters $\mathbf{a} = (a_1, \ldots, a_{12})$, with constant interest rate $r = 0.02$ and with initial prices $\mathbf{S}(0) = (100, 50)$.
...
In particular, for any choice of parameters $\mathbf{a}$, we can fix $T=1$ and $\mathbf{w}=(1,2)$ and use the C++ pricer to approximate the option prices $C(1,K)$ for $K=100,105,\ldots,300$ and, for each of the option prices obtained, calculate the implied volatility $\sigma_\text{BS}^\mathbf{a}(K)$.
...
We can then optimise the parameters such that the implied volatility fit market data as closely as possible. That is, we want to find the parameters $\mathbf{a}$ that minimise the error between the values $\sigma_\text{BS}^\text{market}(K)$ and $\sigma_\text{BS}^\mathbf{a}(K)$. More precisely, we take this error to be
\begin{align*}
