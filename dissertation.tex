...
\begin{theorem}\label{thm:cond-exp-measurable-independent}
  ...
  \end{itemize}
  For any bounded Borel function $f : \mathbb{R}^2 \to \mathbb{R}$, we have
  \begin{align*}
    \mathbb{E}(f(X,Y) \mid \mathcal{F}^W_s) = g_f(X),
  \end{align*}
  almost surely, where $g_f : \mathbb{R} \to \mathbb{R}$ is a bounded Borel function given by
  \begin{align*}
...
% FIXME: Provide counterexample to European basket options being Markov to justify the restriction to a local volatility model. + under the filtration generated by S too.
...
\begin{theorem}
  The discounted price $\widetilde{H}$ of a European put basket option with weights $\mathbf{w}$ is a Markov process in the filtered probability space $(\Omega, \mathcal{F}, \mathcal{F}^\mathbf{S}_t, Q)$.

  \begin{proof}
    Fix the weights $\mathbf{w}$. We denote $B_\mathbf{w}$ as simply $B$ for convenience. Let $f$ be a bounded Borel function and let $0 \le s < t \le T$. We have to prove that
    \begin{align*}
    ...
    % FIXME: $\widetilde{H}$ is also a $Q$-martingale under this coarser filtration.
...
\begin{theorem}
  The discounted price $\widetilde{H}$ of a European call basket option with weights $\mathbf{w}$ is a Markov process.
