...
\tomcomment{Still work in progress}
...
\begin{align*}
  ...
  &\ \ \ \ \ - \frac{1}{2} \left(f(x+h_x,y) - 2 f(x,y) + 2f(x-h_x,y)\right)\\
  &\ \ \ \ \ - \frac{1}{2} \left(f(x,y+h_y) - 2 f(x,y) + 2f(x,y-h_y) \right) \Bigg) + o(h_x^2 + h_y^2).
\end{align*}
...
\begin{align*}
  ...
  &\ \ \ \ \ + \frac{1}{2} \sum_{i,j,l=1}^{d} w_i w_l c_{ij}(T) c_{lj}(T) \frac{\partial^2}{\partial w_i \partial w_l} C(T,\mathbf{w}).
\end{align*}
Using the results established here, we can approximate the partial derivatives as
\begin{align*}
  \frac{\hat{C}(T,\mathbf{w}) - \hat{C}(T-h_T,\mathbf{w})}{h_T}
  &= \sum_{i=1}^{d} r(T) w_i \hat{\theta}_i(T,\mathbf{w})
    + \frac{1}{2} \sum_{i,j,l=1}^{d} w_i w_l c_{ij}(T) c_{lj}(T) \hat{\phi}_{il}(T,\mathbf{w}).
\end{align*}
FIXME: Edges of the tree

For some $n$, we can break the interval $[0,T]$ into subintervals of equal length $h_T = \frac{T}{n}$. Similarly, for all $i=1,\ldots,d$, we take $h_{w_i}$ such that
\begin{align*}
  FIXME
\end{align*}
for $i=1,\ldots,d$ and $h_T, h_{w_i}$.
...
\begin{align*}
  \hat{\mathbf{s}} &= \sum_{k=1}^{n} \left( - \frac{1}{2} \mathbf{C}(kh) \mathbf{C}(kh)^T h + \mathbf{C}(kh) \mathbf{\Delta}_k \right),
\end{align*}
where $\mathbf{\Delta}_k$ is the vector of increments of the Wiener processes. We can simulate realisations of $\mathbf{\Delta}_k$ since each of the increments of the Wiener processes (for each $k=1,\ldots,n$ and $j=1,\ldots,d$) follow a normal distribution with mean 0 and variance $h$, independent from one another.
