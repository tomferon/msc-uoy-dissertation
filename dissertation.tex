...
\begin{theorem}
  ...
  \begin{proof}
    Let $y > 0$ and et $(f_K)$ be the sequence $f_K(x) = (x-K)^+ \rho(x,T;y,t)$. Note that
    \begin{align*}
      |f_K(x)|
      &\le |x \rho(x,T;y,t)|,
    \end{align*}
    for $x > 0$, and
    \begin{align*}
      \int_0^\infty |x\rho(x,T;y,t)| \mathrm{d}x
      &= \int_0^\infty x\rho(x,T;y,t) \mathrm{d}x \tag{positive integrand}\\
      &= \mathbb{E}_Q(S(T) \mid \mathcal{F}^W_t)
      = y < \infty,
    \end{align*}
    since $S$ is a $Q$-martingale, so $x \rho(x,T;y,t)$ is integrable. Hence, by Lebesgue's Dominated Convergence Theorem, we have
    \begin{align*}
      \lim_{K \to \infty} \int_0^\infty f_K(x) \mathrm{d}x
      &= \int_0^\infty \lim_{K \to \infty} f_K(x) \mathrm{d}x
      = \int_0^\infty 0 \mathrm{d}x = 0.
    \end{align*}
    This holds for all $y \in \mathrm{Im}(S(t))$ so
    \begin{align*}
      \lim_{K \to \infty} C(T,K)
      &= \lim_{K \to \infty} \left(e^{-\int_t^T r(s) \mathrm{d}s} \int_\mathbb{R} (x-K)^+ \rho(x,T;S(t),t) \mathrm{d}x\right) \\
      &= e^{-\int_t^T r(s) \mathrm{d}s} \lim_{K \to \infty} \int_0^\infty f_K(x) \mathrm{d}x = 0.
    \end{align*}

    FIXME: Explain 0 to infty

    FIXME: Fix the mess with y and S(t)
  \end{proof}
\end{theorem}
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    \begin{align*}
      \int_0^K r(T) k &\rho(k,T;S(t),t) \mathrm{d}k\\
      &= r(T) e^{\int_t^T r(s) \mathrm{d}s} \int_0^K k \frac{\partial^2}{\partial k^2} C(T,k) \mathrm{d}k \tag{by \eqref{eq:dupire-rho}}\\
      &= r(T) e^{\int_t^T r(s) \mathrm{d}s} \left( K \frac{\partial}{\partial K} C(T,K) - \int_0^K \frac{\partial}{\partial k} C(T,k) \mathrm{d}k \right)\\
      &= r(T) e^{\int_t^T r(s) \mathrm{d}s} \left( K \frac{\partial}{\partial K} C(T,K) - C(T,K) \right),
    \end{align*}
    by integration by parts. Hence, substituting in the Fokker-Planck equation and integrating twice with respect to $K$, we get
    \begin{align*}
    ...
    \end{align*}
    where $\alpha(T), \beta(T)$ are the constant of integration of each integration.
    % FIXME: Assumption that lim dC/dT = 0
    Dividing by the discounting factor and noticing that the terms in $r(T)C(T,K)$ cancel out, we have
    \begin{align*}
