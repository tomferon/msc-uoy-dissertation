...
By $C^{1,2}$, we denote the class of functions $f : \mathbb{R}^2 \to \mathbb{R}$ that are continuously differentiable in the first argument and twice-continuously differentiable in the second, i.e. both $f_x(x,y)$ and $f_{yy}(x,y)$ exist and are continuous.

\begin{theorem}[It\^o formula]
  (\cite{capinski_stochastic_2012}, Theorem 4.22)\label{thm:ito-formula}
%%%%%%%%%%%%%
\comment{citation missing}
%%%%%%%%%%%%%
  If $F : [0,T] \times \mathbb{R} \to \mathbb{R}$ is in $C^{1,2}$
  %%%%%%%%%%%%%
...
\begin{definition}
  A \textbf{multidimensional It\^o process} is a stochastic process $\mathbf{X}(t) = (X_1(t), X_2(t), \ldots, X_d(t))$ that satisfies
  \begin{align*}
    \mathrm{d}X_i(t) = a_i(t) \mathrm{d}t + \sum_{j=1}^n b_{ij}(t) \mathrm{d}W_j(t), \text{ for } i=1,\ldots,d
  \end{align*}
  where $W_j(t), j=1,\ldots,n$ are $n$ Wiener processes, $a_i(t), i=1,\ldots,d$ are stochastic processes such that $\int_0^T |a_i(t)| \mathrm{d}t < \infty$ and $b_{ij} \in \mathcal{P}^2$ for $i=1,\ldots,d$ and $j=1,\ldots,n$.
\end{definition}

\begin{theorem}[Multidimensional It\^o formula]
  (\cite{capinski_blackscholes_2012}, Theorem 6.10)\label{thm:ito-formula-multi}
...
\begin{lemma}(\cite{capinski_blackscholes_2012}, Proposition 6.16)\label{lem:exp-sums-martingale}
...
\begin{lemma}\label{lem:ito-integral-girsanov}
  ...
  Then
  \begin{align*}
    \int_0^t X(s) \mathrm{d}W^Q(s) = \int_0^t X(s) \theta(s) \mathrm{d}s + \int_0^t X(s) \mathrm{d}W(s).
  \end{align*}
%%%%%%%%%%%%%
...
\end{align}
where $W_j(t), j=1,\ldots,d$ are independent Wiener processes with respect to the (physical) probability $P$. Some further assumptions are made by the model.

\begin{assumption}\label{ass:drift-vol-regularity}
  The processes $\mu_i$ and $c_{ij}$  for all $i,j=1,\ldots,d$ are $\mathcal{F}^\mathbf{W}_t$-adapted with continuous paths and are bounded by a deterministic constant.
\end{assumption}
...
\begin{theorem}
  ...
  \begin{proof}
    Let $i = 1,\ldots,d$. By the multidimensional It\^o formula (theorem \ref{thm:ito-formula-multi}) with $F(t,x) = \ln x$ so that $F_t(t,x) = 0, F_x(t,x) = \frac{1}{x}$ and $F_{xx} = \frac{-1}{x^2}$, we have
    \begin{align*}
      \mathrm{d}F(t,S_i(t))
...
Therefore, the discounted asset prices are martingales under $Q$ by lemma \ref{lem:exp-sums-martingale} and since
