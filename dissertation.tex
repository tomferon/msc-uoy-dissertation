...
\begin{definition}
  A \textbf{stopping time} with respect to the filtration $\mathcal{F}_t$ is a random variable $\tau$ such that the event $\{\tau \le t\}$ is in the $\sigma$-field $\mathcal{F}_t$ for all $t \in [0,T]$.
\end{definition}

\begin{definition}
  Given $X \in \mathcal{P}^2$, a \textbf{localising sequence} for $X$ in $\mathcal{M}^2$ is a sequence of stopping times $(\tau_n)_{n \in \mathbb{N}}$ with respect to $\mathcal{F}^W_t$ such that
  \begin{itemize}
    \item $\tau_{n+1} \ge \tau_{n}$ for all $n$,
    \item $\mathbf{1}_{[0,\tau_n]} X \in \mathcal{M}^2$ for all $n$, and
    \item $P(\cup_{n} \{\tau_n = T\}) = 1$.
  \end{itemize}
\end{definition}

\begin{theorem}\label{thm:p2-localising-sequence-limit}
  (\cite{capinski_stochastic_2012}, Theorem 4.16)
  Let $X \in \mathcal{P}^2$ and $(\tau_n)$ be any localising sequence for $X$ in $\mathcal{M}^2$. If $(X_n)_{n \ge 1}$ is the sequence of stochastic processes given by
  \begin{align*}
    X_n(t) = \mathbf{1}_{[0,\tau_n]}(t) X(t),
  \end{align*}
  so that $X_n \in \mathcal{M}^2$ for all $n$, then the sequence of continuous martingales
  \begin{align*}
    M_n(t) = \int_0^t X_n(s) \mathrm{d}W(s)
  \end{align*}
  as in \eqref{eq:stochastic-integral-process} converges almost surely to a stochastic process $Y$ with continuous paths that does not depend on the choice of localising sequence $(\tau_n)$.
\end{theorem}

\begin{proposition}\label{prop:p2-localising-sequence-existence}
  (\cite{capinski_stochastic_2012}, Proposition 4.14)
  Let $X \in \mathcal{P}^2$. The sequence $(\tau_n)_{n \in \mathbb{N}}$ given by
  \begin{align*}
    \tau_n = \inf \left\{ t \in [0,T] : \int_0^t X(s)^2 \mathrm{d}s \ge n \right\}
  \end{align*}
  with $\inf \emptyset = T$ is a localising sequence for $X$ in $\mathcal{M}^2$.
\end{proposition}
...
\begin{definition}
  ...
  \end{align*}
  with $M_n$ and $Y$ as in Theorem \ref{thm:p2-localising-sequence-limit}.
\end{definition}

Note that Proposition \ref{prop:p2-localising-sequence-existence} ensures the existence of a localising sequence and Theorem \ref{thm:p2-localising-sequence-limit} states the limit does not depend on which localising sequence we choose, so the stochastic integral is well-defined for integrands in $\mathcal{P}^2$.

\begin{theorem}\label{thm:stochastic-integral-split-m2}
  (\cite{oksendal_stochastic_2003}, Theorem 3.2.1 (i))
  Let $0 \le a < b < c \le T$ and $X \in \mathcal{M}^2$. Then
  \begin{align*}
    \int_a^c X(t) \mathrm{d}t = \int_a^b X(t) \mathrm{d}W(t) + \int_b^c X(t) \mathrm{d}W(t).
  \end{align*}
\end{theorem}

\begin{theorem}\label{thm:stochastic-integral-split}
  Let $0 \le a < b < c \le T$ and $X \in \mathcal{P}^2$. Then
  \begin{align*}
    \int_a^c X(t) \mathrm{d}t = \int_a^b X(t) \mathrm{d}W(t) + \int_b^c X(t) \mathrm{d}W(t).
  \end{align*}

  \begin{proof}
    By Proposition \ref{prop:p2-localising-sequence-existence}, we have a localising sequence $(\tau_n)$ for $X$ in $\mathcal{M}^2$ and the sequence of processes $X_n(t) = \mathbf{1}_{[0,\tau_n]}$ is in $\mathcal{M}^2$. Since
    \begin{align*}
      \mathbb{E}\left(\int_0^T (\mathbf{1}_{[a,c]} X_n(t))^2 \mathrm{d}t\right)
      &= \mathbb{E}\left(\int_0^T \mathbf{1}_{[a,c]} X_n(t)^2 \mathrm{d}t\right)\\
      &\le \mathbb{E}\left(\int_0^T X_n(t)^2 \mathrm{d}t\right) < \infty,
    \end{align*}
    then $(\tau_n)$ is also a localising sequence for $\mathbf{1}_{[a,c]}X$ is $\mathcal{M}^2$.

    Therefore,
    \begin{align*}
      \int_a^c X(t) \mathrm{d}W(t)
      &= \int_0^T \mathbf{1}_{[a,c]} X(t) \mathrm{d}W(t)\\
      &= \lim_{n \to \infty} \int_0^T \mathbf{1}_{[a,c]} X_n(t) \mathrm{d}W(t)\\
      &= \lim_{n \to \infty} \int_a^c X_n(t) \mathrm{d}W(t)\\
      &= \lim_{n \to \infty} \int_a^b X_n(t) \mathrm{d}W(t) + \lim_{n \to \infty} \int_b^c X_n(t) \mathrm{d}W(t) \tag{by Theorem \ref{thm:stochastic-integral-split-m2}}\\
      &= \int_a^b X(t) \mathrm{d}W(t) + \int_b^c X(t) \mathrm{d}W(t).
    \end{align*}
  \end{proof}
\end{theorem}
...
\begin{definition}
  ...
  \begin{align}\label{eq:sde-init-value}
    \mathbf{X}(t) &= \mathbf{X}(s) + \int_s^t a(u, \mathbf{X}(u)) \mathrm{d}u + \int_s^t b(u, \mathbf{X}(u)) \mathrm{d}\mathbf{W}(u),\\
    \mathbf{X}(s) &= \mathbf{x},\notag
  \end{align}
  for some $a : [0,T] \times \mathbb{R}^{d} \to \mathbb{R}^d$, $b : [0,T] \times \mathbb{R}^{d} \to \mathbb{R}^{d \times n}$ and $\mathbf{x} \in \mathbb{R}^d$.
\end{definition}
...
\begin{theorem}\label{thm:sde-solution}
  ...
  \end{itemize}
  Then \eqref{eq:sde-init-value} has a unique solution with continuous paths that is adapated to the filtration generated by $\mathbf{W}(t)$ for $t \ge s$ and such that
  \begin{align*}
    \mathbb{E}\left(\int_0^T\right) FIXME
  \end{align*}
\end{theorem}
...
\begin{theorem}[Martingale Representation Theorem]\label{thm:martingale-representation}
  (\cite{abed_martingale_2005}, Theorem 6)
  Let $\mathbf{W}(t)$ be a $d$-dimensional Wiener process and $M(t)$ be a square-integrable martingale. Then there exists an $\mathcal{F}^\mathbf{W}_t$-adapted process $\mathbf{\Gamma}(t)$ such that
  \begin{align*}
...
\begin{theorem}\label{thm:bs-solution}
  ...
  \begin{proof}
    ...
    Since, by Assumption \ref{ass:drift-vol-regularity}, $\mu_i$ and $c_{ij}$ are bounded by a deterministic constant for all $i,j=1,\ldots,d$, both $u$ and $v$ are Lipschitz with linear growth with respect to $\mathbf{x}$. Therefore, by Theorem \ref{thm:sde-solution}, the solution is unique.
  \end{proof}
\end{theorem}
...
\begin{theorem}
  ...
  \begin{align*}
    M(t) = \exp \left( - \frac{1}{2} \sum_{j=1}^d \int_0^t \theta_j(s)^2 \mathrm{d}s - \sum_{j=1}^d \int_0^t \theta_j(s) \mathrm{d}W_j(s) \right).
  \end{align*}
  If $M(t)$ is a martingale under $P$, then the measure $Q$ with density $\frac{\mathrm{d}Q}{\mathrm{d}P} = M(T)$ is a risk-neutral probability, i.e. it is equivalent to $P$ and the processes $\tilde{S}_i, i=1,\ldots,d$ are martingales under $Q$.
  %%%%%%%%%%%%%
...
%%%%%%%%%%%%%
  ...
  \begin{proof}
    ...
    By Lemma \ref{lem:bs-exponential-martingale} with $a_j(t) \coloneq - c_{ij}(t)$ and Assumption \ref{ass:drift-vol-regularity}, $\tilde{S}_i(t)$ is thus a martingale under $Q$ for all $i=1,\ldots,d$ as required.
...
\begin{definition}
  ...
  \begin{align*}
    \tilde{X}(t) = \frac{X(t)}{A(t)} = e^{-\int_0^t r(s) \mathrm{d}s} X(t).
  \end{align*}
\end{definition}

Note that since $e^{-\int_0^0 r(s) \mathrm{d}s} = 1$, a process agrees with its dicounted process at time 0, i.e. $X(0) = \tilde{X}(0)$.
...
\begin{definition}
  ...
  \begin{align*}
    V(T) = H_\text{payoff}
  \end{align*}
  and such that its discounted value process $\tilde{V}$ is a martingale under $Q$.
\end{definition}
...
\begin{theorem}\label{thm:bs-replicability}
  ...
  \begin{proof}
    ...
    \end{align*}
    since $H(T)$ is $\mathcal{F}^\mathbf{W}_T$-measurable. Hence, since $H(T)$ is square-integrable, then so is $\tilde{V}(T) = e^{-\int_0^T r(s)\mathrm{d}s} H(T)$.
    ...
    \begin{align}\label{eq:bs-repl-deriv-gamma}
      \tilde{V}(t)
      &= \tilde{V}(0) + \int_0^t \mathbf{\Gamma}(s) \cdot \mathrm{d}\mathbf{W}^Q(s).
    \end{align}

    Since $\Gamma_i \in \mathcal{M}^2$, the stochastic integrals have continuous paths by Definition \ref{def:stochastic-integral-as-process} and, since $A(t)$ is also continuous, then $V(t) = A(t) \tilde{V}(t)$ has continuous paths.

    Note that this process $\tilde{V}$ exists whether there exists a replicating strategy or not, being the conditional expectation under $Q$ of the discounted payoff, so we are not in fact using the existence of a replicating strategy in order to construct one, however it may seem at first sight.
    ...
    \begin{align*}
      ...
      &= \sum_{i=1}^d A(t)^{-1} x_i(t) \mathrm{d}S_i(t) + A(t)^{-1} y(t) r(t) A(t) \mathrm{d}t - y(t) A(t) r(t) A(t)^{-1} \mathrm{d}t\\
        &\ \ \ \ - \sum_{i=1}^d x_i(t) S_i(t) r(t) A(t)^{-1} \mathrm{d}t \tag{substituting $V(t)$ and by \eqref{eq:multi-bs-eq-risk-free}}\\
      ...
      &= \sum_{i=1}^d [ A(t)^{-1} x_i(t) r(t) S_i(t) \mathrm{d}t + \sum_{j=1}^d A(t)^{-1} x_i(t) c_{ij}(t) S_i(t) \mathrm{d}W^Q_j(t) \\
        &\ \ \ \ \ \ \ \ \ - x_i(t) S_i(t) r(t) A(t)^{-1} \mathrm{d}t ] \tag{by Theorem \ref{thm:bs-solution}}\\
      &= \sum_{i,j=1}^d x_i(t) c_{ij}(t) \tilde{S}_i(t) \mathrm{d}W^Q_j(t)
      = \mathbf{C}(t) \mathbf{\theta}(t) \cdot \mathrm{d}\mathbf{W}^Q(t)
    \end{align*}
    where $\theta_i(t) = x_i(t) \tilde{S}_i(t)$ for $i=1,\ldots,d$.
    ...
    \begin{align*}
      x_i = \frac{(\mathbf{C}^{-1} \mathbf{\Gamma})_i}{\tilde{S}_i},
    \end{align*}
...
\begin{theorem}\label{thm:bs-derivative-pricing}
  ...
  \begin{align}\label{eq:bs-derivative-pricing}
    \tilde{H}(t) = \mathbb{E}_Q(\tilde{H}(T) \mid \mathcal{F}^\mathbf{W}_t)
  \end{align}
  for all $t \in [0,T]$.

  \begin{proof}
    By Theorem \ref{thm:bs-repl-strat-derivative-prices}, we have that $H(t) = V(t)$ for all $t \in [0,T]$. Multiplying both sides by the discounting factor $\exp\left(-\int_0^t r(s) \mathrm{d}s\right)$, we also have that $\tilde{H}(t) = \tilde{V}(t)$ for all $t \in [0,T]$.

    Since $(x_1, \ldots, x_d, y)$ is a replicating strategy, its discounted value $\tilde{V}$ is a Q-martingale. Therefore,
    \begin{align*}
      \tilde{H}(t) = \tilde{V}(t)
      = \mathbb{E}_Q(\tilde{V}(T) \mid \mathcal{F}^\mathbf{W}_t)
      = \mathbb{E}_Q(\tilde{H}(T) \mid \mathcal{F}^\mathbf{W}_t).
    \end{align*}
  \end{proof}
\end{theorem}
...
\begin{align*}
  H(0) = \mathbb{E}_Q\left(\tilde{H}(T)\right)
...
This section is based on \textcite{pavliotis_stochastic_2014}. We work in a filtered probability space $(\Omega, \mathcal{F}, \mathcal{F}_t, P)$ without stating it explicitly as to not clutter the argument unecessarily.
...
\begin{definition}
  An \textbf{It\^o diffusion (process)} is an It\^o process of the form
  \begin{align*}
    \mathbf{X}(t) &= \mathbf{X}(0) + \int_0^t a(s, \mathbf{X}(s)) \mathrm{d}s + \int_0^t b(s, \mathbf{X}(s)) \mathrm{d}\mathbf{W}(s),
  \end{align*}
  for some Borel functions $a : [0,T] \times \mathbb{R}^{d} \to \mathbb{R}^d$, $b : [0,T] \times \mathbb{R}^{d} \to \mathbb{R}^{d \times n}$ and $\mathbf{x}_0 \in \mathbb{R}^d$ satisfying the conditions of Theorem \ref{thm:sde-solution}.
\end{definition}

\begin{theorem}
  It\^o diffusions are Markov processes.

  \begin{proof}
    Let $\mathbf{X}$ be an It\^o diffusion and let $0 \le s < t \ge T$. We have
    \begin{align*}
      \mathbf{X}(s) &= \mathbf{X}(0) + \int_0^s a(u,\mathbf{X}(u)) \mathrm{d}u + \int_0^s b(u,\mathbf{X}(u)) \mathrm{d}\mathbf{W}(u),\\
      \mathbf{X}(t) &= \mathbf{X}(0) + \int_0^t a(u,\mathbf{X}(u)) \mathrm{d}u + \int_0^t b(u,\mathbf{X}(u)) \mathrm{d}\mathbf{W}(u),
    \end{align*}
    for some functions $a,b$ as in the definition. By subtracting the second by the first, using Theorem \ref{thm:stochastic-integral-split}, we obtain
    \begin{align*}
      \mathbf{X}(t) &= \mathbf{X}(s) + \int_s^t a(u,\mathbf{X}(u)) \mathrm{d}u + \int_s^t b(u,\mathbf{X}(u)) \mathrm{d}\mathbf{W}(u).
    \end{align*}

    Seeing the previous equation as an SDE with initial condition $\mathbf{X}(s) = \mathbf{x}$, we can see that Theorem \ref{thm:sde-solution} implies that there is a unique solution $\mathbf{X}(t,\omega)$ satisfying the SDE for that particular choice of $\mathbf{x} \in \mathbb{R}^d$. Let $\phi(\mathbf{x},t,\omega)$ denote these solutions as a function of $\mathbf{x}$.

    By Theorem \ref{thm:sde-solution}, the solution is adapted to the filtration generated by $W(t)$ for $t \ge s$. In other words, it is independent from $\mathcal{F}^\mathbf{W}_s$.

    Hence, for all bounded Borel function $f$,
    \begin{align*}
      \mathbb{E}(f(\phi()))
    \end{align*}
    \begin{align*}
      \mathbb{E}(f(\mathbf{X}(t)) \mid \mathcal{F}^W_s) = FIXME
    \end{align*}
  \end{proof}
\end{theorem}
...
\begin{theorem}[Chapman-Kolmogorov Equation]\label{thm:fp-chapman-kolmogorov}
  ...
  \begin{proof}
    ...
    \begin{align*}
      ...
      &= \mathbb{E}(\mathbf{1}_\Gamma(\mathbf{X}(t)) \mid \mathcal{F}^\mathbf{X}_s)\\
      &= \mathbb{E}(\mathbb{E}(\mathbf{1}_\Gamma(\mathbf{X}(t)) \mid \mathcal{F}^\mathbf{X}_u) \mid \mathcal{F}^\mathbf{X}_s) \tag{tower property}\\
...
\begin{theorem}[Fokker-Planck Equation]\label{thm:fokker-planck}
  Let $\mathbf{X}(t)$ be an It\^o process satisfying
  \begin{align*}
