...
Taking $\mathbf{a}(t) = (a_1(t),\ldots,a_d(t))$ and $\mathbf{B}(t) = [b_{ij}(t)]_{i=1,\ldots,d;j=1,\ldots,n}$ and
\begin{align*}
  \int_0^t \mathbf{B}(s) \cdot \mathrm{d}\mathbf{W}(s) = \left[
...
%%%%%%%%%%%%%
\begin{align*}
  \mathbf{X}(t) = \mathbf{X}(0) + \int_0^t \mathbf{a}(s) \mathrm{d}s + \int_0^t \mathbf{B}(s) \cdot \mathrm{d}\mathbf{W}(s)
\end{align*}
or to be even terser, in its so-called \textbf{stochastic differential} notation:
\begin{align*}
  \mathrm{d}\mathbf{X}(t) = \mathbf{a}(t) \mathrm{d}t + \mathbf{B}(t) \cdot \mathrm{d}\mathbf{W}(t).
\end{align*}
...
\begin{definition}
  ...
  \begin{align}\label{eq:sde-init-value}
    \mathbf{X}(t) &= \mathbf{X}(0) + \int_0^t u(s, \mathbf{X}(s)) \mathrm{d}s + \int_0^t v(s, \mathbf{X}(s)) \cdot \mathrm{d}\mathbf{W}(s),\\
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    Moreover, by \ref{ass:drift-vol-regularity}, both $\mathbf{\mu}$ and $\mathbf{C}$ have continuous paths that are bounded on the closed bounded interval $[0,T]$. Therefore, they are both FIXME
  \end{proof}
\end{theorem}
